    ...
    <el-card>
      ...
      <el-row>
        ...
        <el-col :span="20">
          ...
          <el-button size="mini" type="primary" icon="el-icon-edit" @click="exportTaxYear">导出</el-button>
        </el-col>
      </el-row>
          </div>
      <el-row>
...
import { getYearTax,exportYearTax } from "@/frame_src/api/MonthTaxExport";
export default {
  data() {
    return {
      statistics: [],
  ...
  methods: {
    ...
    getList() {
      getYearTax(this.listQuery).then(response => {
  ...
  exportTaxYear() {
      exportYearTax(this.listQuery).then(response => {
    ...
    exportExcelInfo() {
      import("@/frame_src/vendor/Export2Excel").then(excel => {
        ...
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: this.listQuery.S_WorkDate.substr(0, 7) + "年度地税报表"
        });
      });
    },
